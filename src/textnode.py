...
text_types_valid = ['text', 'bold', 'italic', 'code', 'link', 'image']
...
class TextNode:
    ...
    def split_nodes_delimiter(old_nodes, delimiter, text_type):
        new_nodes_list = []
        if text_type not in text_types_valid:
        ...
        for node in old_nodes:
            ...
            for i in range(len(node_split)):
                if not node_split[i]:
                    continue
                if i%2 == 1:
                    new = TextNode(node_split[i], text_type)
                else:
                    new = TextNode(node_split[i], text_type_text)
                new_nodes_list.append(new)
